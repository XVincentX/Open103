...
4)	A LINK PROTOCOL DATA UNIT (LPDU) of this companion standard contains not more than one APPLICATION SERVICE DATA UNIT (ASDU).
...
private:
	...
	typedef union IFI_
	{
		...
		struct
		{
			...
			unsigned char InformationNumber;         /* The full range <0..255> is used independently in the control direction as well as in the monitor direction. */
...
public:
	...
	typedef struct cp56Time2A_
	{
		...
		friend std::ofstream& operator<<(std::ofstream &out, const cp56Time2A_& object)
		{
			...
			out << object.GetDayMonth() << "/" << object.GetMonth() << "/" << object.GetYear() << "," << object.GetHours() << ":" << object.GetMinutes() << ":" << secs << "." << millisecs;
			return out;
		}
		...
		cp56Time2A_(unsigned short milliseconds,unsigned char minutes, unsigned char hours, unsigned char dayweek, unsigned char daymonth, unsigned char month, unsigned char years, unsigned char SU) :
		...
		inline void AddMilliseconds(unsigned short add)
		{
			this->Milliseconds+=add;
		}

		inline unsigned short GetMilliseconds()         const
		{
			return this->Milliseconds;
		}
		inline unsigned short GetMinutes()                      const
		{
			return this->Minutes & 0x3F;
		}
		inline unsigned short GetHours()                        const
		{
			return this->Hours & 0x1F;
		}
		inline unsigned short GetSummerTime()           const
		{
			return this->Hours & 0x80;
		}
		inline unsigned short GetMonth()                        const
		{
			return this->Month & 0xF;
		}
		inline unsigned short GetYear()                         const
		{
			return 2000+(this->Year & 0x7F);
		}
		inline unsigned short GetDayWeek()                      const
		{
			return this->Day & 0xE0;
		}
		inline unsigned short GetDayMonth()                     const
		{
			return this->Day & 0x1F;
		}
	private:
	...
	{
		AnalogChannel_(std::string ch_id, std::string ph, std::string ccbm,std::string uu, unsigned int channelCode) : uu(uu), channelCode(channelCode), COMTRADEChannel(ch_id,ph,ccbm)
		{
		}
	...
	typedef struct DigitalChannel_ : public COMTRADEChannel
	{
		...
		DigitalChannel_(std::string ch_id, std::string ph, std::string ccbm, std::string y, unsigned char FType, unsigned char Ifi) : Ifi(Ifi), FType(FType), y(y), currVal(0), COMTRADEChannel(ch_id,ph,ccbm)
		{
		}
		std::string y;
	...
	IEC8705103Manager() {}
	IEC8705103Manager(CommunicationPort *p, const unsigned char address) : linklayermanager(DBG_NEW IEC87052Manager(p,address)), _address(address),fType(None)
	{
	...
	bool StationInit()
	{
		...
		if (!this->linklayermanager->ResetRemoteLink())
			return false;
		if (!this->linklayermanager->StatusLink())
			return false;
		...
		{
			if (lastHeader.DataUnitIdentifier.SepDui.CauseOfTrasmission >= 3 && lastHeader.DataUnitIdentifier.SepDui.CauseOfTrasmission <= 5)
			{
				TRACEENDL("Communication reset");
				if (!this->linklayermanager->UserDataClass(1))
					return false;
				return true;
			}
			else
		...
		if (lastHeader.DataUnitIdentifier.SepDui.CauseOfTrasmission != 4 && lastHeader.DataUnitIdentifier.SepDui.CauseOfTrasmission != 3)
		{
		...
		//unsigned char Sq = ADSUHeader::extractBitRange(lastHeader.DataUnitIdentifier.SepDui.VariableStructureIdentifier,0,7);
		unsigned char numInfo = ASDUHeader::extractBitRange(lastHeader.DataUnitIdentifier.SepDui.VariableStructureIdentifier,7,1);
		...
		//TRACEENDL("Function type for this protection:" + Logger::ToString(lastHeader.InformationObjectIdentifier.SepIfi.FunctionType))
	...
	inline bool TimeSync(const time_t *time, timeval *tz)
	{
		...
		cp56Time2A t2a(
			static_cast<unsigned short>(t.tm_sec * 1000),
			static_cast<unsigned char>(t.tm_min),
			static_cast<unsigned char>(t.tm_hour),
			static_cast<unsigned char>(t.tm_wday),
			static_cast<unsigned char>(t.tm_mday),
			static_cast<unsigned char>(1 + t.tm_mon),
			static_cast<unsigned char>(t.tm_year),
			static_cast<unsigned char>(t.tm_isdst)
			);
	...
	*/
	inline bool CommandTrasmission(Command Command,unsigned char DCO,unsigned char RII,int FTYPE)
	{
		...
		case OvercurrentProtection:
			if (Command >= 16 && Command <= 19)
				break;
		case LineDifferentialProtection:
			if (Command == 16 || Command == 18 || Command == 19)
				break;
		...
		*(reinterpret_cast<ASDUHeader*>(buffer)) = ASDUHeader(DUI(20,129,20,this->_address),IFI((FunctionType)FTYPE,Command));
	...
	inline bool EnableTestMode()
	{
		return false;
	}                                                      //Siprotec cannot handle this.
	...
	{
		memcpy((void*)&lastHeader,pAsdu,ASDUHeaderSize);
		return lastHeader.DataUnitIdentifier.SepDui.TypeIdentification >= 23 && lastHeader.DataUnitIdentifier.SepDui.TypeIdentification <= 31;

	}
	/*Query protection for generic service and data/write functions*/
	inline bool GenericService()
	{
		return false;
	}                                                       //Siprotec cannot handle this.
	...
	inline bool StationStart()
	{
		...
		if (!this->GeneralInterrogation(this->_address))
			return false;
	...
	*/
	static bool SaveToComtrade(std::string filename, std::string StationName, unsigned short StNum, const LPDISTURBANCE data, const AnalogChannel achannels[8],unsigned short AChannelCount, DigitalChannel *dchannels, unsigned short DChannelCount, std::string linefreq, std::string nsamples = "1")
	{
		...
		{
			short max = *std::max_element(data->ChannelList.Channels[achannels[i].channelCode].SDV,data->ChannelList.Channels[achannels[i].channelCode].SDV+data->ChannelList.ChannelElements);
			short min = *std::min_element(data->ChannelList.Channels[achannels[i].channelCode].SDV,data->ChannelList.Channels[achannels[i].channelCode].SDV+data->ChannelList.ChannelElements);

			file << i+1 << "," << achannels[i].ch_id <<","<< achannels[i].ph << "," << achannels[i].ccbm << "," << achannels[i].uu << "," <<  1/(32768) *data->ChannelList.Channels[achannels[i].channelCode].RFA << ","
				<< 0 << "," << 0 <<  "," << min << "," << max << "," << data->ChannelList.Channels[achannels[i].channelCode].RPV << "," << data->ChannelList.Channels[achannels[i].channelCode].RSV
				<< ",S" << std::endl;
		}

		for (unsigned short i = 0; i < DChannelCount; i++)
			file << i << "," << dchannels[i].ch_id << ","<<dchannels[i].ph << "," << dchannels[i].ccbm <<"," << dchannels[i].y << std::endl;

		file << linefreq << std::endl;    //Frequency.?
		file << "1" << std::endl;         //Sampling frequency is the same in all measures.
		file << 1 / (static_cast<float>(data->SamplingTime) / 1000000.f) << "," << data->ChannelList.ChannelElements << std::endl;
				...
				if (data->TagsList.TagsHeader[j].TAP == 0)
				{
					for (int w = 0; w < data->TagsList.TagsHeader[j].NOT; w++)
					{
							...
							data->TagsList.TagsHeader[j].TagsValue[w].In == dchannels[k].Ifi)
						{
							...
							//TRACEENDL(Logger::ToString(dchannels[k].FType)+"-"+ Logger::ToString(dchannels[k].Ifi) + std::string(" initial value:") + Logger::ToString(dchannels[k].currVal));
						}
					}
				}
			}
					...
					if (data->TagsList.TagsHeader[j].TAP == i)
					{
						...
						for (int k = 0; k < DChannelCount; k++)
						{
							for (int w = 0; w < data->TagsList.TagsHeader[j].NOT; w++)
							{
								...
								{
									dchannels[k].currVal = data->TagsList.TagsHeader[j].TagsValue[w].DIP - 1;
									//TRACEENDL(Logger::ToString(dchannels[k].FType)+"-"+ Logger::ToString(dchannels[k].Ifi) + std::string(" updated to:") + Logger::ToString(dchannels[k].currVal));
								}
							}
						}
...
private:
	...
	inline bool DisturbanceRequest(const void *pAsdu)
	{
		...
		//TRACEENDL("Fault record found. FAN: "+Logger::ToString((*(unsigned short *)&FAN))+" SOF:"+Logger::ToString((int)SOF));
		...
		*/
		...
		for (int i = 0; i < buffer[8]; i++)
		{
			...
			*/
			...
			this->DCurrent.TagsList.TagsHeader[this->DCurrent.TagsList.TagsCount].TagsValue[i].FType = buffer[11+(3*i)+0];
	...
	inline bool DisturbanceChannelGet(const void *pAsdu)
	{
		...
		*cf = *tf;
		...
		for (int i = 0; i < tmp->NDV; i++)
		{
			...
			this->DCurrent.ChannelList.Channels[tmp->ACC].SDV[i+this->DCurrent.ChannelList.Channels[tmp->ACC].Header.NFE] = sval;
